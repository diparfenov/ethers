...
const Primitives = () => {
    ...
    const handleSetTrue = async (isTrue) => {
        // вызываем signer для взаимодейсвтия с контрактом а не для чтения
        try {
    ...
    const handleSetSmallBytesSubmmit = async (event) => {
        event.preventDefault();
        //кодируем строку из инпута в массив байтов с помощью спец функции
        const utf8Encode = new TextEncoder();
        const arr = utf8Encode.encode(smallBytesRef.current.value);
        console.log("arr: ", arr);
        //и с помощью еще одной функции это приводится к байтам
    ...
    const handleSetNameSubmmit = async (event) => {
        ...
        try {
            const primitivesWithSigner = await getPrimitivesWithSigner();
            const tx = await primitivesWithSigner.setName(
    ...
    return (
        <Layout>
            ...
            {smallUint != undefined && (
                <h3>Small uint: {smallUint.toString()}</h3>
            )}
            <form onSubmit={handleSetSmallUintSubmmit}>
                ...
                <input
                    ...
                    //   ref={smallUintRef}
                    onChange={(event) =>
                        setSmallNumberValue(event.target.value)
                    }
